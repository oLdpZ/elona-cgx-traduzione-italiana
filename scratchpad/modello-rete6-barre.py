# -*- coding: utf-8 -*-
"""Il modello di lotto con la RETE 6 che vede anche il `//` (100a sessione).

E' `scratchpad/modello-rete4.py` con **due** correzioni alla rete 6, e nessun
altra differenza.

⚠️⚠️ **La prima: HSP ha DUE commenti di riga, e la rete ne guardava uno.**
`;` e `//` spengono tutt'e due il resto della riga, e la rete 6 chiedeva solo
`lstrip().startswith(';')`. `custom_autopick.hsp:200`-`:217` tiene spenti con
`//` due selettori interi — ` zombie ` e ` dragon's ` — che `estrai` estraeva
come se fossero vivi. E' la **quinta famiglia di riga morta** dopo il `;`, il
blocco `/* */`, il ramo `if ( jp )` e l'`if ( 0 )`. ✅ Misurata prima di toccare
la rete, su tutto il sorgente (`scratchpad/_100-commento-barre.py`): **9 righe
con una `lang()` dopo un `//`**, di cui **una gia' tradotta** —
`command.hsp:17515`, lavoro speso su testo che il giocatore non legge. E' la
**sesta rete che si corregge**, dopo la 8, la 4, la 9, la 6 e la 4.

⚠️ **La seconda: la rete leggeva lo scratch invece dello strumento.**
`scratchpad/commenti-blocco.py` e `strumenti/commenti.py` sono la stessa
funzione in due copie, e solo la seconda ha dei test. Il modello adesso importa
`strumenti.commenti`, cosi' la correzione del `//` vale per la rete **e** per
`larghezze.py`, che di quella funzione si serve per non prendere la larghezza di
un riquadro da codice morto.

💡 Stessa forma di `modello-rete4.py` della 57a e di `modello-chiave-lunga.py`
della 41a: il modello nuovo vive in un file suo, e i lotti dopo ci si appoggiano.
"""
import collections
import glob
