...
| ⭐⭐ `assembla-lotto.py` | **copia le reti VERBATIM dal modello** e cambia solo le cinque costanti che devono cambiare — `USCITA`, `DA, A`, `RINVIATE`, `SORGENTE` e il percorso dell'estrazione — poi **rilegge quel che ha scritto** e lo confronta col modello carattere per carattere. Rende meccanica la regola «si copia il file, non si riscrive a memoria», che la ripresa ripeteva da cinque sessioni senza poterla imporre. Vuole due file scritti a mano (`testaNNN.py` col docstring, `reseNNN.py` col dizionario) più un `rinviateNNN.py` facoltativo. ⚠️ Il **sesto argomento è il file `.hsp`**, e senza si intende `proc.hsp`. ⭐⭐ **Dalla 108ª sa dichiarare la zona per INSIEME di righe** (`--righe righeNNN.py`), riscrivendo due righe del modello unico invece di far nascere un secondo modello: serviva alle descrizioni di `db_item.hsp`, sparse per settantacinquemila righe. E `--lavoro`, `--fase`, `--hsp` rendono dichiarabili tre convenzioni che erano dedotte e rotte da sei sessioni. ⚠️ **Il modello da usare è `_102-rese-card-06.py`**, non `modello-rete4.py`: reti identiche a quelle dell'ultimo lotto delle carte (zero righe di differenza da `_106-rese-card-30.py`) e in più l'ancora `# rete 5: l'accento`, che i lotti dal 07 in poi hanno perso copiandosi l'un l'altro | per **ogni** lotto |
...
## La 108ª — il primo lotto di descrizioni

| file | a cosa serve | quando si lancia |
|---|---|---|
| ⭐ `_108-accento-decomposto.py` | la **rete 5 portata fuori dal lotto**: gli accenti decomposti nel dizionario intero, **0 su 23.404**. Serve perché la riga `unicodedata.normalize('NFC', …)` è caduta dai lotti delle carte fra il 06 e il 07 e nessuno strumento la sostituisce — `accenti.TABELLA` sostituisce le vocali **precomposte**, quindi una «é» scritta come `e` + U+0301 le passa davanti intatta. ⚠️ La prova al contrario **dice dove si accende**, non stampa un ✅ | fra le verifiche d'apertura, ed è la ventesima |
...
niente**. Uno strumento che non ha ancora avuto un caso da giudicare non è uno
strumento che funziona: è uno strumento non provato.
