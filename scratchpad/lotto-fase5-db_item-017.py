# -*- coding: utf-8 -*-
"""111a - Lotto 017 di `db_item.hsp`: il rapporto degli ELMI e dei CAPPELLI.

`FILTER_HELM`, `description(3)`: **15 righe del sorgente, 15 firme**, 14
giapponesi distinti.

### ⭐ TRE PAROLE PER LA TESTA, E RESTANO TRE

兜 «elmo», 帽子 «cappello», ヘルメット «casco» — il katakana, cioe' il moderno,
stessa mossa di アーマー «armatura» contro 鎧 «corazza» nel lotto 015. ウィッグ
e' «parrucca», che i nomi non identificati dicono gia'.
...
⭐ Ed e' la **gemella** di `:51643` del lotto 015 (使うと見た目が変わる服だ, il
Clothworld): stessa costruzione, stessa coda 何度でも使用することができる, gia'
«Si può usare sempre.» nel dizionario.

### ⭐ TRE RITROVAMENTI, E NESSUNO E' UNA SCELTA

- 見えない者 e' gia' **«chi non si vede»**, dalla pistola resa nel lotto 013
  (「見えない者さえ打ちぬく拳銃だ」 -> «Una pistola che passa da parte a parte
  anche chi non si vede»). Quindi `:89080` e' «Un elmo che fa vedere chi non si
  vede».
...
(:130912), piu' 軽装向けの盾 (:101050) e 騎士用の盾 (:127207) del lotto 014.
In giapponese sono tre sfumature; in italiano sono tutte «per», e forzarne una
diversa direbbe qualcosa che il giapponese non dice.

ⓘ `:100070` e' 重量がある兜だ, **non** 重い: altra espressione, altra resa («di
un certo peso»), come 非常に重い e とても重い restano due.
"""
import collections
import glob
...
USCITA = 'lavoro/fase5-db_item-017.jsonl'
RIGHE = {
    43047, 43115, 65401, 66980, 72421, 76184, 80427, 89080, 99875, 99940,
    100005, 100070, 100135, 130847, 130912,
}
