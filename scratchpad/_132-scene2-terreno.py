...
`{txt}`   scene.hsp:452-473. Ogni riga del sorgente si disegna **verbatim**,
          ...
          ⭐ Tarato su tre schermate del giocatore il 2026-09-03 (scena 0):
          passo fra le righe 20 px, centro a 1279 px su `windoww/2` = 1280,
          e **8,03 / 8,05 / 8,12 px per carattere** -- cioe' esattamente gli
          8 px che il renderer da' per scontati. ⚠️ NON si eredita il 7,7 di
          `larghezze.py`: quello e' lo stesso font a una dimensione diversa.
          ⚠️⚠️ `windoww` e' **configurazione** (`config.hsp:144`), non una
          costante: quella macchina gioca a 2560, dove non stringe niente.
          Il budget si prende dal minimo dichiarato da `config.txt`, 800 px.
          ⭐ E una riga vuota dentro un `{txt}` non arriva a schermo:
          `scene.hsp:410-415` la cancella. Confermato sulla schermata --
          tre capoversi separati da righe vuote si vedono come 13 righe di
          seguito, senza stacco.
...
`{chat_N}` scene.hsp:427-436 -> `*chat` -> `*chat_scene` -> `chatMore buff`.
          ...
          `wy + 43 + cnt*19` (chat.hsp:25728) e la voce «continua» sta a
          `wy + wh - 56 - keyrange*19 + 2` = `wy + 307` con una voce sola:
          `43 + 19*cnt < 307` da' `cnt <= 13`, cioe' **14 righe**.
          ⚠️ Il primo giro scriveva 13, contando l'indice invece delle righe.
          ⚠️ Qui il vincolo non e' la larghezza, e' **l'altezza**, e il
          riquadro e' fisso a 600x380 (chat.hsp:25230) quale che sia
          `windoww`: questo budget, al contrario di quello della targa, non
          dipende dalla configurazione del giocatore.
...
# (`strlen*4` per centrare, `strlen*8` per la targa). Misurato a schermo: 8,05.
PIXEL_PER_CARATTERE = 8.0
# `windoww` e' configurazione; si tara sul minimo che `config.txt` dichiara
FINESTRA = 800
BUDGET_TARGA = (FINESTRA - 80) // 8                      # 90
BUDGET_BORDO = int(FINESTRA / PIXEL_PER_CARATTERE)       # 100, il testo vero

# init.hsp:1279 -- talk_conv buff, 56 - en*3
COLONNA_CHAT = 53
# chat.hsp:25728 (43 + 19*cnt) contro :25161 (wy + 307): cnt <= 13, righe 14
RIGHE_CHAT = 14
...
def referto(nome="scene2.hsp"):
    ...
    # ⚠️ il referto non stampa un esito booleano: stampa **di quanto** il
    # blocco piu' stretto e' vicino al soffitto. Uno zero senza la ragione
    # dello zero non e' un risultato (lezione della 107a).
    stretti = [m for m in misurati if m[0] >= RIGHE_CHAT - 2]
    print("   a due righe o meno dal soffitto: %d" % len(stretti))
    for n, caratteri, scena, testo in misurati[:3]:
        print("     scena %-4s %2d righe su %d, margine %d, %4d caratteri: %s..."
              % (scena, n, RIGHE_CHAT, RIGHE_CHAT - n, caratteri, testo[:50]))
